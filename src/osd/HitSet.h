...
public:
  ...
  explicit TempHitSet()
    : count(0), cache_hit(0), decay_period(1024), baseline(0),
      old_avg_temp(0), exp_avg_temp(1<<8) {
  ...
  explicit TempHitSet(const TempHitSet::Params *p)
    : count(0), cache_hit(0), baseline(0), old_avg_temp(0),
      exp_avg_temp(1<<8) {
    set_dp(p->get_dp());
    uint32_t now_sec = ceph_clock_now().tv.tv_sec;
    rh.reset(new RankHistogram(decay_period, now_sec));
  ...
  TempHitSet(const TempHitSet &o)
    : hits(o.hits), count(o.count), cache_hit(0), 
      decay_period(o.decay_period), baseline(o.baseline),
      old_avg_temp(0), exp_avg_temp(1<<8) {
    uint32_t now_sec = ceph_clock_now().tv.tv_sec;
    rh.reset(new RankHistogram(decay_period, now_sec));
    last_decay = now_sec;
